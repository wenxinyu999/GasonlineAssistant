...
public class HomeActivity extends exitsystem implements View.OnClickListener {
    ...
    private void initView(){
       ...
       // customBarChart1=(LinearLayout)findViewById(R.id.customBarChart1) ;
        ...
        double Zhichu=db.searchzongw(accnum,ym,"金额");
        double Shouru=db.searchzongw(accnum,ym," ");

        new AlertDialog.Builder(HomeActivity.this)
                .setTitle("温馨提示：")
                .setMessage("主人，根据你的记账情况总结如下："+"\n"+"当月你的总支出金额为 "+String.valueOf(Zhichu)+"元"+"\n"+"总加油量 "+String.valueOf(Zhichu/10)+"L")
    ...
    private void initData(){
        ...
        photos.add(R.mipmap.p1);
        photos.add(R.mipmap.p2);
        photos.add(R.mipmap.p3);
        photos.add(R.mipmap.p4);
        photos.add(R.mipmap.p5);
        photos.add(R.mipmap.p6);
...
//    }
    ...
    public void paintzhe(){
        //设置图表的描述
        lineChart2.setDescription("近七天的加油情况");
        //设置x轴的数据
        int numX = 7;
        String s_result=db.judgeproperid(accnum,nowdate," ");
        int tt_id=0;
        if (s_result.equals("yes")){
             tt_id=db.searchproperid(" ",accnum,nowdate);
        }
        else{
            int t_id=db.searchid();
            db.adddataintotal(t_id,accnum,nowdate," ",0,0,0,0,0);
             tt_id=db.searchproperid(" ",accnum,nowdate);
        }
        //设置y轴的数据
        float[] sdata1=new float[7];
        float[] datas1 = db.searchdatedataone(tt_id,accnum," ");//数据  收入

        int size=db.searchcountone(tt_id,accnum," ");
        int i=0;
        int j=0;
        if (size<7){
            for (i=0;i<7;i++){
        ...
        String z_result=db.judgeproperid(accnum,nowdate,"金额");
        int zt_id=0;
        if (z_result.equals("yes")){
            zt_id=db.searchproperid("金额",accnum,nowdate);
            System.out.println("zt_id"+zt_id);
            System.out.println("总支出="+db.inoutdata(accnum,nowdate,"金额"));
        }
        else{
            int t_id=db.searchid();
            db.adddataintotal(t_id,accnum,nowdate,"金额",0,0,0,0,0);
            zt_id=db.searchproperid("金额",accnum,nowdate);
        }
        float[] zdata1=new float[7];
        float[] datas2 =db.searchdatedataone(zt_id,accnum,"金额") ;//数据  支出
        int zsize=db.searchcountone(tt_id,accnum," ");
        int w=0;
        int z=0;
        if (zsize<7){
            for (w=0;w<7;w++){
        ...
        //设置折线的名称
        LineChartManager.setLineName("金额");
        //设置第二条折线y轴的数据
        LineChartManager.setLineName1(" ");
